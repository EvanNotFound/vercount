...
- The React package should remain independently publishable.
...
- Add the existing React package under `/packages/react`.
...
### 3. Keep the React package as a standalone publishable package inside the monorepo

`/packages/react` should keep its own package metadata and build flow so it can still be published independently.
...
### 5. Bring the React package into the repo as imported source, then normalize docs and references
...
- **Docs may become inconsistent during transition** → Update all obvious references to the external React package repo as part of the migration.
...
4. Bring the React package into `/packages/react`.
