...
import { getStoredPagePaths, pruneStoredPagePaths } from "@/utils/counter";
import { safeDecodeURIComponent } from "@/utils/url";
import { db } from "@/db";
import { domains } from "@/db/schema";
import { and, eq } from "drizzle-orm";

// GET handler - Get stored page paths for a domain
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch known page paths from the per-domain inventory
    const hostSanitized = domain.name;
    const storedPaths = await getStoredPagePaths(hostSanitized);
    const pagesPipeline = kv.pipeline();
    storedPaths.forEach((storedPath) => {
      pagesPipeline.get(`pv:page:${hostSanitized}:${storedPath}`);
    });

    const pageValues = await pagesPipeline.exec();
    const stalePaths: string[] = [];
    const paths = storedPaths.filter((storedPath, index) => {
      if (pageValues[index] === null) {
    ...
    if (stalePaths.length > 0) {
      await pruneStoredPagePaths(hostSanitized, stalePaths);
    }
