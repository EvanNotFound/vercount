...
function isSubdomainOf(subdomain: string, parentDomain: string): boolean {
  ...
  const isSubdomain =
    normalizedSubdomain !== normalizedParentDomain &&
    normalizedSubdomain.endsWith(normalizedParentDomain) &&
    normalizedSubdomain.charAt(
      normalizedSubdomain.length - normalizedParentDomain.length - 1,
    ) === ".";

  logger.debug(
    `Checking if ${normalizedSubdomain} is a subdomain of ${normalizedParentDomain}: ${isSubdomain}`,
  );
...
export const domainService = {
    ...
    try {
      ...
      if (existingDomain) {
        ...
        if (existingDomain.userId === userId) {
          ...
          return {
            success: false,
            message: "This domain has already been registered by another user",
          };
        }
      }
      ...
      logger.info(
        `Checking if ${normalizedDomain} is a subdomain of any verified domains for user ${userId}`,
      );

      // Check if any of the user's verified domains is a parent domain of the new domain
      const parentDomain = userDomains.find((domain) =>
        isSubdomainOf(normalizedDomain, domain.name),
      );

      if (parentDomain) {
        logger.info(
          `${normalizedDomain} is a subdomain of verified domain ${parentDomain.name}, auto-verifying`,
        );
      }
  ...
  async verifyDomainViaDNS(domain: any) {
    try {
      logger.info(
        `Attempting to verify domain: ${domain.name} with DNS lookup for _vercount.${domain.name}`,
      );
      ...
      try {
        ...
        logger.info(`DNS TXT records found for _vercount.${domain.name}:`, {
          txtRecords,
        });
      } catch (dnsError) {
        logger.error(`DNS lookup failed for _vercount.${domain.name}`, {
          error: dnsError,
        });
        return {
          success: false,
          message:
            "DNS verification failed: Could not find TXT record. Please ensure you've added the _vercount TXT record to your DNS settings.",
        };
      }
      ...
      const verified = txtRecords.some((record) => {
        ...
        logger.info(
          `Comparing TXT record: "${joined}" with expected: "${expectedValue}"`,
        );
      ...
      if (!verified) {
        return {
          success: false,
          message:
            "Domain verification failed: TXT record found but value doesn't match. Please ensure you've added the correct TXT record value.",
        };
      }
      ...
      logger.error("Error verifying domain via DNS", {
        error,
        domainId: domain.id,
        domain: domain.name,
      });
      return {
        success: false,
        message:
          "Failed to verify domain via DNS. Please check your DNS configuration and ensure the TXT record is properly set.",
      };
    }
  },
  ...
  async verifyDomainViaFile(domain: any) {
    try {
      logger.info(
        `Attempting to verify domain: ${domain.name} via file verification`,
      );
      ...
      logger.info(
        `File content: "${normalizedContent}", Expected: "${expectedContent}"`,
      );
      ...
      if (normalizedContent !== expectedContent) {
        return {
          success: false,
          message:
            "File verification failed: File content doesn't match expected value. Please ensure the file contains the correct verification string.",
        };
      }
      ...
      logger.error("Error verifying domain via file", {
        error,
        domainId: domain.id,
        domain: domain.name,
      });
      return {
        success: false,
        message:
          "Failed to verify domain via file. Please check that the verification file exists and is accessible.",
      };
    }
  },
  ...
  async getCountersForDomain(domainName: string) {
    try {
      ...
      const [sitePv, siteUv] = await Promise.all([
        kv.get(sitePvKey),
  ...
  async updatePageViewCounter(
    ...
    try {
      ...
      logger.error("Error updating page view counter", {
        error,
        domainName,
        path,
      });
  ...
  async removeMonitoredPage(domainId: string, path: string) {
    try {
      ...
      return {
        success: true,
        message: "Monitored page removed (KV data preserved)",
      };
    } catch (error) {
