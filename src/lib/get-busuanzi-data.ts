...
export async function getBusuanziSiteUVData(host: string, path: string) {
  ...
  if (data) {
    ...
    await kv.set(`live_site_uv:${host}`, JSON.stringify({ site_uv: 0 }), {
      ex: EXPIRATION_TIME,
    });
    logger.error(
...
export async function getBusuanziSitePVData(host: string) {
  ...
  if (data) {
    ...
    logger.info(`PV data retrieved and stored for ${host}`);
    return sitePv;
  } else {
    await kv.set(`live_site_pv:${host}`, JSON.stringify({ site_pv: 0 }), {
      ex: EXPIRATION_TIME,
    });
    logger.error(
...
export async function getBusuanziPagePVData(host: string, path: string) {
  ...
  if (dataNoSlashResult && dataSlashResult) {
    ...
    logger.info(`Page PV data retrieved and stored for ${host}${path}`);
    return pagePv;
